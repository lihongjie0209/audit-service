...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/audit/records/create]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.audit.v1.AuditService/Record]")
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/audit/records/create" {
		t.Fatalf("HTTPPaths=%q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.audit.v1.AuditService/Record" {
...
func TestConfig_DatabaseRequiresApplicationUpstream(t *testing.T) {
	...
	cfg.Database.DSN = "postgres://audit"
